...
import { removeStuff, updateHODFields } from '../../../redux/HODRelated/HODHandle';
...
const ViewHOD = () => {
    ...
    useEffect(() => {
        dispatch(getUserDetails(HODID, address));
    }, [dispatch, HODID])
    ...
    const [HODSchool, setHODSchool] = useState('');
    ...
    useEffect(() => {
        if (userDetails) {
            ...
            setHODSchool(userDetails.school || '');
    ...
    const submitHandler = (event) => {
        ...
        dispatch(updateUser(fields, HODID, address))
            .then(() => {
                dispatch(getUserDetails(HODID, address));
            })
            .catch((error) => {
    ...
    const deleteHandler = () => {
        ...
        // dispatch(deleteUser(HODID, address))
        //     .then(() => {
        //         navigate(-1)
        //     })
    }
    ...
    const removeSubAttendance = (subId) => {
        dispatch(updateHODFields(HODID, { subId }, "RemoveHODSubAtten"))
            .then(() => {
                dispatch(getUserDetails(HODID, address));
            })
    }
        ...
        const renderTableSection = () => {
            return (
                <>
                                    ...
                                    <StyledTableRow>
                                        ...
                                        <StyledTableCell align="center">
                                            ...
                                            <Button variant="contained" sx={styles.attendanceButton}
                                                onClick={() => navigate(`/Admin/subject/HOD/attendance/${HODID}/${subId}`)}>
                                                Change
                                            </Button>
                                        </StyledTableCell>
                                    </StyledTableRow>
                    ...
                    <Button variant="contained" color="error" startIcon={<DeleteIcon />} onClick={() => removeHandler(HODID, "RemoveHODAtten")}>Delete All</Button>
                    <Button variant="contained" sx={styles.styledButton} onClick={() => navigate("/Admin/HOD/HOD/attendance/" + HODID)}>
                        Add Attendance
                    </Button>
                </>
            )
        ...
        const renderTableSection = () => {
            return (
                <>
                    ...
                    <Button variant="contained" sx={styles.styledButton} onClick={() => navigate("/Admin/HOD/HOD/marks/" + HODID)}>
                        Add Marks
                    </Button>
                </>
            )
